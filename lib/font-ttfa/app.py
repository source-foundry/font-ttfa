...
#------------------------------------------------------------------------------------
# c.cmd = Primary command (font-ttfa <primary command>)
# c.cmd2 = Secondary command (font-ttfa <primary command> <secondary command>)
#
...
def main():
    ...
    if not c.command_suite_validates():
        from font-ttfa.settings import usage as font-ttfa_usage
        print(font-ttfa_usage)
        sys.exit(1)
    #------------------------------------------------------------------------------------------
    ...
    if c.help():      # User requested font-ttfa help information
        from font-ttfa.settings import help as font-ttfa_help
        print(font-ttfa_help)
        sys.exit(0)
        ...
        from font-ttfa.settings import app_name, major_version, minor_version, patch_version
    ...
    # 		  friend_of_spam = c.arg('--with')    # user enters font-ttfa spam --with eggs
    # 		  print('spam and ' + friend_of_spam) # prints 'spam and eggs'
    # elif c.cmd == 'naked':
    #     if c.flag("--language"):
    #         lang = c.flag_arg("--language")     # user enters font-ttfa naked --language=python
